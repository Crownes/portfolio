...
// - вызов экспортной процедуры происходит из проведения расходных платёжных документов
// - пример такого кода
...
Процедура ОбработкаВыгрузкиВозврата(Документ, Отказ, ДопПараметры = Неопределено) Экспорт
	...
	ВыгрузитьВозврат(Документ, Отказ, ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Документ.РасшифровкаПлатежа[0].ОбъектРасчетов,"Объект"), Документ.СуммаДокумента);
...
Процедура ВыгрузитьВозврат(Документ, Отказ, Знач ОснованиеПлатежа, Знач СуммаВозврата)
	Если ТипЗнч(ОснованиеПлатежа) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		...
		///======================================================================================================================
		// Получение текущих оплат в заказе, инициализация таблицы значений для изменённых платежей
		Оплаты = ЗаказВCRM["orders"][0]["payments"];
		СуммаОставшегосяВозврата = СуммаВозврата;
		
		ИзменённыеПлатежи = Новый ТаблицаЗначений;
		ИзменённыеПлатежи.Колонки.Добавить("АйдиПлатежа", Новый ОписаниеТипов("Строка"));
		ИзменённыеПлатежи.Колонки.Добавить("Оплата", Новый ОписаниеТипов("Соответствие"));
		ИзменённыеПлатежи.Колонки.Добавить("СуммаВозврата", Новый ОписаниеТипов("Число"));
		...
		Для Каждого СтрОплаты Из Оплаты Цикл
			...
			Если СтатусОплаты = "paid" Или СтатусОплаты = "perenos" Тогда
				Если СуммаОплаты > СуммаОставшегосяВозврата Тогда
					ИзменённаяОплата = XMLСтрока(СуммаОплаты - СуммаОставшегосяВозврата);
				...
				Платеж.Вставить("amount", ИзменённаяОплата);
				Если Число(ИзменённаяОплата) = 0 Тогда
				...
				ИзменилиНа = XMLСТрока(СуммаОплаты - Число(ИзменённаяОплата));
				...
				НовСТРТЗ = ИзменённыеПлатежи.Добавить();
				НовСТРТЗ.Оплата = перемен;
				НовСТРТЗ.АйдиПлатежа = XMLСтрока(СтрОплаты["id"]);
				НовСТРТЗ.СуммаВозврата = СуммаОплаты - Число(ИзменённаяОплата);
				///======================================================================================================================
				// Возврат выгрузится с типом самой последней изменённой оплаты
		...
		//======================================================================================================================
		// Вызов сторонней библиотеки для выгрузки скомпонованных данных - изменённых платежей в заказе
		СуммаВозвращённого = 0;
		Для Каждого Стр Из ИзменённыеПлатежи Цикл
			...
			Если СоответствиеОтвета.Получить("success") <> Неопределено Тогда
				СуммаВозвращённого =СуммаВозвращённого + Стр.СуммаВозврата;
			Иначе
				Продолжить;
			КонецЕсли;
		...
		Платеж = Новый Соответствие;
		Платеж.Вставить("amount", XMLСтрока(СуммаВозвращённого));
